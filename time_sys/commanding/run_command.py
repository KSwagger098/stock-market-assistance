...
def run_commands(user_dict: dict, yahoo_scrap, check_symbol, send_function, command: str, chat_id: str) -> tuple[dict, str]:
    """
    ...
    if split_commands[0] == '!commands' and len(split_commands) == 1:
        ...
        user_dict, msg = add_function(split_commands, check_symbol, send_function, user_dict, chat_id)
    elif split_commands[0] == '!remove':
        user_dict, msg = remove_function(split_commands, check_symbol, send_function, user_dict, chat_id)
...
def add_function(split_commands : list[str], check_symbol, send_function, user_dict : dict, chat_id : str) -> tuple[dict, str]:
    """
    ...
    if len(split_commands) > 1:
        for stock_symbol in split_commands[1:]:
            ...
            if check_symbol(stock_symbol, '!add'):
                user_dict[chat_id][1].append(str(stock_symbol).upper())
                send_function(f'{str(stock_symbol).upper()} has been added to your list of stock!', chat_id)
            else:
                send_function(f'{stock_symbol} does not exist!', chat_id)
    else:
        ...
        send_function(msg, chat_id)
    return user_dict, ''


def remove_function(split_commands : list[str], check_symbol, send_function, user_dict : dict, chat_id : str) -> tuple[dict, str]:
    """
    ...
    if len(split_commands) > 1:
        for stock in split_commands[1:]:
            if str(stock).upper() in user_dict[chat_id][1]:
                ...
                send_function(f'{str(stock).upper()} has been removed from your list of stock!', chat_id)
            else:
                send_function(f'{stock} cannot be removed since it\'s not present in your list of stocks.\nPlease refer to !stock to check your list of stocks.', chat_id)
    else:
